...
class EmailService {
  ...
  async sendPasswordResetEmail(userEmail, userData) {
    try {
      const { firstName, resetCode } = userData;

      const mailOptions = {
        from: process.env.EMAIL_USER,
        to: userEmail,
        subject: 'Password Reset Code - SmartLaptop',
        html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h2 style="color: #2c3e50;">Password Reset Request</h2>
          <p>Dear ${firstName || 'User'},</p>
          <p>You requested a password reset for your SmartLaptop account.</p>
          <p>Your reset code is:</p>
          <div style="font-size: 28px; font-weight: bold; letter-spacing: 4px; padding: 20px; background: #f3f4f6; border-radius: 8px; text-align: center; margin: 20px 0; color: #111827;">
            ${resetCode}
          </div>
